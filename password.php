...
if (isset($_POST['email']) && isset($_POST['recoverytext'])) {
    ...
    $stmt = $conn->prepare("SELECT firstname, lastname, recoverytext, email, city, state, country, password FROM user WHERE email = ? AND recoverytext = ?");
    if (!$stmt) {
    ...
    if ($result->num_rows > 0) {
        ...
        echo '<script>
            ...
            setTimeout(function() {
                window.location.href = "signin.php"; // Redirect to dashboard or another page
